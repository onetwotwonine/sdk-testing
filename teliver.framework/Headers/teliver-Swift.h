...
SWIFT_CLASS("_TtC7teliver12QKInitailize")
@interface QKInitailize : NSObject
...
/// @abstract Register for Push Notification with the username.
...
+ (void)registerForPushNotificationForUser:(NSString * _Nonnull)username withToken:(NSString * _Nonnull)token;
...
/// @param user     A Teliver Tracker object of a users to be tracked. Refer @TeliverTracker for more information.
+ (void)startTrackingForUser:(TeliverTracker * _Nonnull)user;
/// @abstract Start tracking operator using the tracking Id provided by the operator.
...
+ (void)startTrackingForUsers:(NSArray<TeliverTracker *> * _Nonnull)users;
...
+ (void)stopTrackingForTrackingIds:(NSArray<NSString *> * _Nonnull)trackingIds;
/// @abstract Stop tracking operator using the tracking Id provided by the operator.
...
+ (void)stopTrackingForTrackingId:(NSString * _Nonnull)trackingId;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
@optional
- (void)didRecieveErrorOnTracing:(NSString * _Nullable)error;
@end
